...
# ── url_extractor helpers ────────────────────────────────────────────────────

def _extract_slugs(content: str, pattern: str, denylist: list[str], count: int,
                   flags: int = 0) -> list[str]:
    seen: set[str] = set()
    results: list[str] = []
    for m in re.finditer(pattern, content, flags):
        slug = m.group(0)
        if any(d in slug for d in denylist):
            continue
        if slug not in seen:
            seen.add(slug)
            results.append(slug)
...
def _build_url_extractor_successors(slugs: list[str], node_meta: dict) -> list[NodeSpec]:
    url_template  = node_meta.get("url_template", "https://huggingface.co/{slug}")
    detail_goal   = node_meta.get("detail_goal",  "Extract key details from this page.")
    next_skill    = node_meta.get("next_skill",   "comparator")
    detail_labels = [f"detail_{i + 1}" for i in range(len(slugs))]
    detail_nodes  = [
        NodeSpec(skill="browser", inputs=[],
                 metadata={"label": lbl, "url": url_template.format(slug=slug), "goal": detail_goal})
        for lbl, slug in zip(detail_labels, slugs)
    ]
    next_node = NodeSpec(
        skill=next_skill,
        inputs=[f"n:{lbl}" for lbl in detail_labels],
        metadata={"label": "compare"},
    )
    formatter_node = NodeSpec(skill="formatter", inputs=["USER_QUERY", "n:compare"], metadata={})
    return detail_nodes + [next_node, formatter_node]
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    if skill.name == "url_extractor":
        node_meta = graph_nodes[node_id].get("metadata") or {}
        slug_regex = node_meta.get("slug_regex", r"[A-Za-z0-9_\-\.]+/[A-Za-z0-9_\-\.]+")
        count      = int(node_meta.get("count", 3))
        denylist   = list(node_meta.get("denylist") or [])
        content = ""
        for r in resolved:
            if r.get("kind") == "upstream":
                out = r.get("output") or {}
                content = out.get("content") or ""
                # a11y driver stores extracted slugs in the done-action value;
                # append it so _extract_slugs can find all of them even when
                # the content field only rendered the first card's text.
                for action_turn in out.get("actions") or []:
                    for act in action_turn.get("actions") or []:
                        if act.get("type") == "done" and act.get("value"):
                            content = content + "\n" + str(act["value"])
                if content:
                    break
        slugs      = _extract_slugs(content, slug_regex, denylist, count)
        successors = _build_url_extractor_successors(slugs, node_meta)
        return AgentResult(
            success=bool(slugs),
            agent_name="url_extractor",
            output={"slugs": slugs, "count": len(slugs)},
